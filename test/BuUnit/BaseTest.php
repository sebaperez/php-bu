...
class BaseTest extends BuTest
{
	...
	public function test_find()
	{
		$NAME = "test";
		$sampleobject = SampleClass::add(["name" => $NAME]);
		$objects = SampleClass::find("name = ?", ["name" => $NAME]);
	...
	{
		$NAME = "test";
		$sampleobject1 = SampleClass::add(["name" => $NAME]);
		$sampleobject2 = SampleClass::add(["name" => $NAME]);
		$samplecomposed = SampleClassMultiplePK::add([
			"id1" => $sampleobject1->getValue("sampleclass_id"),
			"id2" => $sampleobject2->getValue("sampleclass_id"),
			"name" => $NAME
		...
		$objects = SampleClassMultiplePK::find("name = ?", ["name" => $NAME]);
		$this->assertTrue(count($objects) >= 1);
		$flag = false;
		foreach ($objects as $object) {
			$this->assertEquals($NAME, $object->getValue("name"));
